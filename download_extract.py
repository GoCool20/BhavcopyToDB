...
def cleanup_session_directory(dirs):
    """Remove the session directory if exists"""
    try:
        if dirs and 'base_path' in dirs:
            shutil.rmtree(dirs['base_path'], ignore_errors=True)
            logging.info(f"Cleaned up session directory: {dirs['base_path']}")
...
def download_today_bhavcopy():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            cleanup_session_directory(dirs)
            return None
